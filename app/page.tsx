...
export default function RootPage() {
  const { isLoaded, user, signIn, signInWithGoogle } = useAuthCtx()
  ...
  useEffect(() => {
    if (isLoaded && user) router.replace('/citas')
  ...
  // Show loader while auth is resolving or user is already logged in (redirecting)
  if (!isLoaded || user || (loading && !error)) {
    return (
